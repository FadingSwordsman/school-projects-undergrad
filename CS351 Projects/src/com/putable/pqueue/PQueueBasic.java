...
public class PQueueBasic implements PQueue
{
	...
	while (canMove)
	{
	    ...
	    if (canMove = insert.compareTo(toMove) < 0)
	    {
		forIndex = toMove.getIndex();
		set(forIndex, toMove);
	    }
	}
    }
    ...
    /**
     * Copy an element from the oldElements to the tree sometimes. Over N
     * iterations, the entire tree is copied. Naively moves forwards in the list
     * and copies that element down. In the event that that element was affected
     * by Heapify up or down, no element will be present, since it will have
     * been placed in the tree already.
     * 
     * @return True if the tree is not complete copying, and false if the old
