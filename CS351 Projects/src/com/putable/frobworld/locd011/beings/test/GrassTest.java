...
public class GrassTest {
	...
	int[][] locationPairs = new int[][] { { 2, 1 }, { 3, 2 }, { 2, 3 },
			{ 1, 2 } };
	...
	public void setUp() {
		...
		GrassSetting grassSetting = new GrassSetting(0, -200, 10, 30, 10, 2,
				100, 40);

		// Ideally, this wouldn't have to be instantiated, and could be mocked
		// instead. This is a design flaw:
		SimulationSettings mockSettings = new SimulationSettings(worldSettings,
				null, null, grassSetting);
	...
	public void testBirthSuccess() {
		...
		assertEquals(world.getSimulationSettings().getGrassSettings()
				.getGrassInitialUpdatePeriod(), newGrass.getUpdatePeriod());
	}
	...
	public void testCrowdOutBirthFailure() {
		...
		for (int x = 0; x < locationPairs.length; x++) {
			...
			for (int y = x + 1; y < locationPairs.length; y++) {
				...
				assertEquals(
						Math.min(oldUpdate * 2, world.getSimulationSettings()
								.getGrassSettings().getGrassMaxUpdatePeriod()),
	...
	private int massGainedNextTurn(Grass g) {
		int grassTax = world.getSimulationSettings().getGrassSettings()
				.getMassTax();
		int fixedOverhead = world.getSimulationSettings().getGrassSettings()
				.getFixedOverhead();
		return ((grassTax * g.getUpdatePeriod()) / (1000 + fixedOverhead));
	}

	private int calculateBirthMass(int oldMass) {
		int percent = world.getSimulationSettings().getGrassSettings()
				.getGrassBirthPercent();
